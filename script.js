...
// আপনার ডিসকর্ড ওয়েব হুক লিঙ্ক এখানে দিন
const DISCORD_WEBHOOK = "https://discordapp.com/api/webhooks/1484489791801266226/Au28_NX5AH_H0b56_4CLgTJ1D_SF35RIsjgJXTUZa0tE-1sg-BKKVtIhjU8dZIP7zDsg";

let startTime = Date.now();
let userIP = "Fetching...";

// ১. ইউজারের আইপি এবং প্রবেশের তথ্য পাঠানো
async function trackUserJoin() {
    try {
        ...
        const message = `🟢 **নতুন সেশন শুরু!**\n👤 আইপি: \`${userIP}\` \n⏰ সময়: ${new Date().toLocaleString()}\n🌐 ব্রাউজার: ${navigator.userAgent.substring(0, 60)}`;
        sendToDiscord(message);
    } catch (e) { console.log("Join error"); }
}

// ২. ডিসকর্ডে মেসেজ পাঠানোর মেইন ফাংশন
...
// ৩. বাটন ক্লিক ট্র্যাকিং (ইউজার কী করছে)
document.addEventListener('click', (e) => {
    const target = e.target.closest('button, a'); // বাটন বা লিঙ্কে ক্লিক করলে
    if (target) {
        const actionText = target.innerText.trim() || target.title || "Unknown Action";
        const msg = `🖱️ **অ্যাক্টিভিটি:** ইউজার (\`${userIP}\`) ক্লিক করেছে: **"${actionText}"**`;
        sendToDiscord(msg);
    }
});

// ৪. বের হওয়ার সময় কতক্ষণ ছিল তা পাঠানো
window.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'hidden') {
        const durationSeconds = Math.round((Date.now() - startTime) / 1000);
        let durationText = durationSeconds > 60 ? Math.floor(durationSeconds/60) + " মিনিট" : durationSeconds + " সেকেন্ড";
        
        const msg = `🔴 **ইউজার লিভ নিয়েছে!**\n👤 আইপি: \`${userIP}\` \n⏱️ মোট সাইটে ছিল: ${durationText}`;
        sendToDiscord(msg);
    } else {
        startTime = Date.now(); // ট্যাব আবার ওপেন করলে টাইম রিসেট হবে
    }
});

// পেজ লোড হলে ট্র্যাকিং শুরু
trackUserJoin();